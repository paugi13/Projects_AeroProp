...
DE_flap=[-20 -15 -10 -5 0 5 10 15 20]; % flap deflection (deg, positive:down)
...
cm_cg_flap = zeros(9,7);
cl_flap = zeros(9,7);

for i=1:length(DE_flap)
% Wing discretization (lenghts are dimensionless with the wing span)

[c4nods,c75nods,chord,s_pan,h,Cm0_y,normals,mac,S] = geo(AR,TR,N,DE25,ETIP,A0p,CM0p,CDP,YF_pos,CF_ratio,DE_flap(i),FlapCorr); 
...
%% Part 2. 
%1. OK
%drag parasito en funci√≥n de alfa
Cd01=Cf*(1+60/((lh/b)^3)+0.0025*(lh/b))*(Ss./S_bal);

%momento de cabeceo en funci√≥n de alfa
...
%3. CL_max 
%Usando distribuciones de lift b√°sico y adicional 

%Compute Cl_max (local) as a Reynolds function using XFLR5
chord_real = chord*bf;
%Re de cada secci√≥n
Re_section = 1.225*100*chord_real/(3.6*1.789e-5);
%Hay una relaci√≥n exponencial entre Re y cl_local maximo. 
...
%4. CD0 - K Model
%Done in ap2
flight_speed=linspace(1,100,100);
CL=(2*weight)./(dens*S_wing*flight_speed.^2);
CD=0.0211.*CL.^2 -0.0063613.*CL +0.10948;
CL_CD=CL./CD;
% figure
% plot(flight_speed,CL_CD)
% title 'L/D en funci√≥n de la velocidad'
% xlabel 'v [m/s]'
% ylabel 'L/D'
% grid on
...
Cm0=0.166;
X_cg=1.3;
cm_cg_flap(i,:)=cm_0-force_coeff(7,:)*(X_ac - X_cg)/(mac*bf)+C_mf2; 
cl_flap(i,:) = force_coeff(7,:);
end

figure
plot(cl_flap(1,:),cm_cg_flap(1,:));
title 'Cm_{cg} en funciÛn de Cl y del angulo de deflexiÛn del flap'
xlabel 'Cl'
ylabel 'Cm_{cg}'

hold on
plot(cl_flap(2,:),cm_cg_flap(2,:));
plot(cl_flap(3,:),cm_cg_flap(3,:));
plot(cl_flap(4,:),cm_cg_flap(4,:));
plot(cl_flap(5,:),cm_cg_flap(5,:));
plot(cl_flap(6,:),cm_cg_flap(6,:));
plot(cl_flap(7,:),cm_cg_flap(7,:));
plot(cl_flap(8,:),cm_cg_flap(8,:));
plot(cl_flap(9,:),cm_cg_flap(9,:));
xlim([-2.5 2])
ylim([-0.6 0.8])
hold off
legend('-20∫', '-15∫', '-10∫', '-5∫', '0∫', '5∫', '10∫','15∫', '20∫');
grid on 