...
%% FLAP CONFIGURATION
opt = input('Add flap to horizontal tail? Y/N (1/0): ');
tailIncidence = input('Tail incidence angle?: ');
...
%% CP Coords
HrootChord = 2.870;
propValue = HrootChord/chord(N/2);
...
save('wing analysis/workspaces/HTailCPCoords', 'HCPCoords', 'HrootChord');

% Tail data
tailClSlope = polyfit(ALPHA, force_coeff(7, :), 1);

disp('TAIL PITCHING MOMENT ANALYSIS');
